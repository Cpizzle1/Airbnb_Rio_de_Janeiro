...
def clean_df(combined_df3):
    """[Takes in Airbnb dataframe and cleans data to be more analysis friendly and engineered features. Fills NaN in guest values with mode value, etc ]

    Args:
        combined_df3 ([Pandas Dataframe]): 

    Returns:
        [4 pandas dataframe]: [1st dataframe = contact method, 2nd dataframe = instant book method, 3rd = book-it method, full combined dataframe]
    """
...
def KDE_plot_maker(df, lst ):
    """[creates multiple graphs for Kernel Density Estimation from input dataframe from AirBNB data]

    Args:
        df ([pandas dataframe]): [cleaned Dataframe from clean_df function]
        lst ([list ]): [list of column names to make graph]
    """
...
def catagorical_stacked_graph_maker(combined_df3, lst2):
    """[Make stacked bar chart from catagorical data from cleaned airbnb data from clean_df function]

    Args:
        combined_df3 ([pandas dataframe]): [cleaned airbnb data from clean_df function]
        lst2 ([list]): [list of column names to make graph from]
    """
